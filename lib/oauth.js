...
const AuthorizeUrl = 'https://oapi.dingtalk.com/connect/oauth2/authorize'
const AccessTokenUrl = 'https://oapi.dingtalk.com/gettoken'
const UserInfoUrl = 'https://oapi.dingtalk.com/user/getuserinfo'
const UserDetailUrl = 'https://oapi.dingtalk.com/user/get'
...
OAuth.prototype.getAccessToken = function(callback) {
  const self = this
  console.log('dingtalk getAccessToken begin')
  this._getAccessToken(this._corpId, function(err, accessToken) {
    console.log('dingtalk getAccessToken result', accessToken)
...
OAuth.prototype.getUserInfo = function(accessToken, code, callback) {
  ...
  console.log('dingtalk getUserInfo', url)
...
OAuth.prototype.getUserDetail = function(accessToken, userTicket, callback) {
  const params = {}
  params['access_token'] = accessToken.access_token
  params['userid'] = userTicket
  // const postData = {}
  // postData['userid'] = userTicket
  const url = UserDetailUrl + '?' + querystring.stringify(params)
  console.log('dingtalk getUserDetail', url)
  wechatRequest(url, callback)
...
function wechatRequest2(url, requestData, callback) {
  utils.url_request(url, utils.postJSON(requestData), function(err, res, body) {
    if (err) return callback(err)
