...
function WechatWorkStrategy(options, verify, getAccessToken, saveAccessToken) {
  ...
  this.name = 'dingtalk'
  this._verify = verify
  this._oauth = new OAuth2(options.corpId, options.suiteSecret, options.agentId, options.suiteKey, _getAccessToken, _saveAccessToken)
  this._callbackURL = options.callbackURL
  this._scope = options.scope
  this._scopeSeparator = options.scopeSeparator || ' '
  this._state = options.state
  this._suiteId = options.suiteKey
...
WechatWorkStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    async.waterfall([
      ...
      function(user, cb) {
        self._oauth.getUserDetail(saveAccessToken, user.user_id, cb)
      }
      ...
      if (profile.userid) {
        profile.userid = corp_id + '_' + profile.userid
      }
